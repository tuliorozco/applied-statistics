...
# ============================
# Capa SwitchLayer (Mixture of Experts)
# ============================
class SwitchLayer(layers.Layer):
    """
    Implementa el mecanismo central de un Switch Transformer:
    gating disperso (top-1) y una pérdida de balanceo de carga para la estabilidad.
    """
    def __init__(self, num_experts: int, d_ff: int, dropout=0.1, aux_loss_weight=0.01, **kwargs):
    ...
    def call(self, x, training=None):
        ...
        # Pérdida de Balanceo de Carga (si está en entrenamiento)
        if training:
            probs = ops.softmax(logits, axis=-1)
            # f_i: Fracción de tokens ruteados al experto i
            f = ops.mean(ops.one_hot(ops.argmax(logits, axis=-1), self.num_experts, dtype='float32'), axis=[0, 1])
            # P_i: Probabilidad promedio del router para el experto i
            P = ops.mean(probs, axis=[0, 1])
            # La pérdida penaliza la distribución desigual
        ...
        flat_x = ops.reshape(x, [B * T, d])
        flat_top1 = ops.reshape(top1, [B * T])
        
        # Inicializar salida
        output = ops.zeros_like(flat_x)
        
        # Procesar cada experto usando tf.cond para compatibilidad con el modo Graph
        for e in range(self.num_experts):
            mask = ops.equal(flat_top1, e)
            
            # --- INICIO DE LA CORRECCIÓN ---
            
            # Define la función a ejecutar si el experto recibe tokens
            def process_expert():
                expert_input = tf.boolean_mask(flat_x, mask)
                expert_output = self.experts[e](expert_input, training=training)
                
                # Prepara un tensor para esparcir los resultados.
                # Esto es más seguro dentro de un tf.cond
                indices = tf.where(mask)
                return tf.tensor_scatter_nd_update(tf.zeros_like(flat_x), indices, expert_output)

            # Define la función a ejecutar si el experto NO recibe tokens
            def skip_expert():
                return tf.zeros_like(flat_x)
            
            # Usa tf.cond para decidir qué función ejecutar en tiempo de ejecución
            branch_output = tf.cond(ops.any(mask), true_fn=process_expert, false_fn=skip_expert)
            
            # Suma la salida de la rama (sea el resultado del experto o ceros) a la salida total
...
class TransformerSwitchBlock(layers.Layer):
    def __init__(self, d_model, num_heads, d_ff, num_experts, dropout=0.1, aux_loss_weight=0.01, **kwargs):
        super().__init__(**kwargs)
        self.mha = layers.MultiHeadAttention(num_heads=num_heads, key_dim=d_model//num_heads, dropout=dropout)
        self.ln1 = layers.LayerNormalization(epsilon=1e-6)
        self.drop1 = layers.Dropout(dropout)
        self.switch = SwitchLayer(num_experts=num_experts, d_ff=d_ff, dropout=dropout, aux_loss_weight=aux_loss_weight)
...
# ============================
def build_switch_transformer_tabular(cont_cols: List[str],
                                     bin_cols: List[str],
                                     cat_cols: List[str],
    ...
    inputs, X = build_tabular_encoder(cont_cols, bin_cols, cat_cols, cat_vocab_sizes, d_model=d_model)
    ...
    logits = layers.Dense(1, name='logits')(cls)
    outputs = layers.Activation("sigmoid", name="sigmoid_output")(logits)
    model = keras.Model(inputs=inputs, outputs=outputs, name='Switch_Transformer_Tabular')
    return model
